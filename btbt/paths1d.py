...
def find_paths_1d(x, psi, Eg_eV, psi_tol=1e-12):
    """For every interior node i: the path length l[i] (cm, inf = no path)
    and the end point as (j_lo[i], w_hi[i]) - it lies between nodes j_lo
    and j_lo+1 at fraction w_hi from j_lo. Uphill direction is taken from
    the centered difference at i; the path fails if psi stops rising
    before it has gained Eg (a local maximum) or the grid ends."""
    N = len(x)
    l = np.full(N, np.inf)
    ...
    for i in range(1, N - 1):
        ...
        target = psi[i] + Eg_eV
        seg = psi[i::s] if s > 0 else psi[i::-1]
...
def nonlocal_generation_1d(x, psi, phin, phip, cvol, Eg_eV, Vt, model):
    ...
    N = len(x)
    l, j_lo, w_hi = find_paths_1d(x, psi, Eg_eV)
    G, F_eff = path_rate(l, Eg_eV, model)
    ok = G > 0
